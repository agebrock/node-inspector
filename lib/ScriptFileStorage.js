...
$class.listScripts = function(rootFolder, callback) {
  ...
  /**
   * This function is called twice so we reuse the cached results
   * on the second call to save time
   */
  if(Array.isArray($class.filecash[rootFolder])){
      return callback(null, $class.filecash[rootFolder]);
  }
  ...
  glob(
    ...
    function(err, result) {
      ...
      /**
       * store result in the filecache to reuse it, for later reuseage
       */
      $class.filecash[rootFolder] = result;
